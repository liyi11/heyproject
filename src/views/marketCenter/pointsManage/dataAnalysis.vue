            ...
            <el-table
              ...
              <el-table-column align="left" label="用户" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                  <p class="font_main">{{scope.row.user_name}}</p>
                </template>
              </el-table-column>
              ...
              <el-table-column align="left" label="积分领取总额（分）" :show-overflow-tooltip="true">
                <template slot-scope="scope">
              ...
              <el-table-column align="left" label="上次领取时间" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                  <span>{{scope.row.last_time |formattime1}}</span>
                </template>
              </el-table-column>
            </el-table>
          </c-content>
...
import { get_point_rule_data_analysis,getArea } from '@/api/xapi'
  ...
  methods: {
    ...
    async getList(page) {
      ...
      if(this.form.download_csv){
        ...
        const res = await get_point_rule_data_analysis(obj)
        if(res.code==200){
        ...
        const {data,count} = await get_point_rule_data_analysis(obj)
