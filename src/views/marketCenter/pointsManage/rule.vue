    ...
    <section class="page_body" v-else>
      <section v-if="!editFlag">
        <section class="addTitle">
          ...
          <c-button>
            <span @click="openXM" class="main_button">创建积分规则</span>
          </c-button>
        </section>
        <section class="page_content">
          <c-content
            ...
            <el-table
              ...
              <el-table-column align="left" label="积分/次" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                  <p>{{scope.row.label_percent}}</p>
                </template>
              </el-table-column>
...
<script>
import { getPointRule,deletePointRule } from '@/api/xapi'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const {data,count} = await getPointRule(obj)
    ...
    handleDel(row){
      this.$layer.confirm(
          ...
          async (index) => {
            const { code } = await deletePointRule({ id: row.id })
            if (code === 200) {
