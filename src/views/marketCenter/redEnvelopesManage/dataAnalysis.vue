        ...
        <section class="page_content">
          <c-content
            ...
            <el-table
              ...
              <el-table-column align="left" prop="name" label="用户" :show-overflow-tooltip="true"/>
              ...
              <el-table-column align="left" label="领取次数" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                  <p>{{scope.row.count}}</p>
                </template>
              </el-table-column>
              <el-table-column align="left" label="红包领取总额（元）" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                  <p>￥{{scope.row.sum_price}}</p>
                </template>
              </el-table-column>
              <el-table-column align="left" label="上次领取时间" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                  <span>{{new Date(scope.row.collection_time)|formattime1}}</span>
                </template>
              </el-table-column>
            </el-table>
          </c-content>
...
import { getCash_red_envelope_total_data_analysis,getArea } from '@/api/xapi'
  ...
  methods: {
    ...
    async getList(page) {
      ...
      if(this.form.download_csv){
        ...
        const res = await getCash_red_envelope_total_data_analysis(obj)
        if(res.code==200){
        ...
        const {data,count} = await getCash_red_envelope_total_data_analysis(obj)
