    ...
    <section class="page_body"  v-else>
      <section v-if="!editFlag">
        <section class="addTitle">
          ...
          <c-button>
            <span @click="openXM" class="main_button">添加红包规则</span>
          </c-button>
        </section>
        <section class="page_content">
          <c-content
            ...
            <el-table
              ...
              <el-table-column align="left" prop="rule_name" label="规则名称" :show-overflow-tooltip="true"/>
              <el-table-column align="left" label="红包" :show-overflow-tooltip="true">
...
<script>
import { getCashRedRule,deleteCashRedRule } from '@/api/xapi'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const {data,count} = await getCashRedRule(obj)
    ...
    handleDel(row){
      this.$layer.confirm(
          ...
          async (index) => {
            const { code } = await deleteCashRedRule({ id: row.id })
            if (code === 200) {
